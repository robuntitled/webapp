...
export async function sendBookingConfirmationEmail(input: {
  ...
  const text = [
    ...
    '',
    'Ogni servizio è prenotato col rispettivo fornitore. NomadLink non è un tour operator.',
  ]
    .filter(Boolean)
    .join('\n');
  ...
  const html = `
    <div style="font-family:Inter,system-ui,sans-serif;max-width:560px;margin:0 auto;color:#0b1220">
      <p style="font-size:12px;letter-spacing:.16em;text-transform:uppercase;color:#7c3aed;font-weight:700">NomadLink</p>
      ...
      <p style="color:#666;font-size:13px;margin-top:24px">Ogni servizio è prenotato col rispettivo fornitore. NomadLink non organizza pacchetti turistici.</p>
    </div>
  `;
